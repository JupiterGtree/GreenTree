...
nginx -t
pm2 jlist | node -e 'let x="";process.stdin.on("data",d=>x+=d).on("end",()=>{const p=JSON.parse(x).find(v=>v.name==="greentree");if(!p||p.pm2_env.status!=="online")process.exit(1)})'
...
[ -w "$(dirname "${ADMIN_DB_PATH:-/var/lib/greentree/admin.db}")" ]
[ -w /var/lib/greentree ]
